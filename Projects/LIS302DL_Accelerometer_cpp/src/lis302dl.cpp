...
void LIS302DL::Init() {
    // Powering on the accelerometer and enabling axes
    Write(CTRL_REG1, 0x47);
    TIM4_ms_Delay(5);
}

void LIS302DL::Write(uint8_t addr, uint8_t data) {
    cs->GPIO_ResetBit(3u); // CS low (PE3)
    for(volatile int i=0;i<200;i++) { __asm volatile ("nop"); }
    spi->SPI_WriteRead(&addr, NULL, 1);
    spi->SPI_WriteRead(&data, NULL, 1);
    cs->GPIO_SetBit(3u); // CS high
}

int16_t LIS302DL::Read(int reg_addr) {
    uint16_t raw_value = spi->SPI_ReadReg(*cs, CS_MASK, (uint8_t)reg_addr, 0x80u);
...
uint8_t LIS302DL::WhoAmI() {
    return spi->SPI_ReadReg(*cs, CS_MASK, WHO_AM_I, 0x80u);
}

void LIS302DL::ModeFallback(uint8_t expected) {
    uint8_t who = WhoAmI();
    if (who != expected) {
        spi->SPI_SetMode(0); // try mode 0
        TIM4_ms_Delay(1);
        who = WhoAmI();
        if (who != expected) {
            spi->SPI_SetMode(3); // restore mode 3
        }
    }
}
