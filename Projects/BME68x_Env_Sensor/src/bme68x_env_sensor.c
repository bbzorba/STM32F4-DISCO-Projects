...
static void bme68x_write_u8(I2C_HandleType *hi2c, uint8_t address, uint8_t reg, uint8_t val)
{
    I2C_Start(hi2c);
    I2C_SendAddress(hi2c, address, 0); // write
    I2C_Write(hi2c, reg);
    I2C_Write(hi2c, val);
    I2C_Stop(hi2c);
}

static uint8_t bme68x_read_u8(I2C_HandleType *hi2c, uint8_t address, uint8_t reg)
{
    uint8_t v;
    ...
    // Read one byte
    I2C_Restart(hi2c);
    I2C_SendAddress(hi2c, address, 1); // read
    I2C_DisableAck(hi2c);
    while (!(hi2c->regs->SR1 & I2C_SR1_RXNE)); // Use hi2c->regs instead of hardcoded I2C_1
    v = (uint8_t)hi2c->regs->DR; // Use hi2c->regs instead of hardcoded I2C_1
    I2C_Stop(hi2c);
    I2C_EnableAck(hi2c);
    return v;
}
...
{
    // Check if device responds first
    I2C_Start(hi2c);
    int ack = I2C_SendAddress(hi2c, address, 0);
    if (!ack) {
        I2C_Stop(hi2c);
        return 0xFF; // Device not responding
    }
    I2C_Stop(hi2c);
    
    // Small delay before actual read
    delay_cycles(1000);
    
    // Now do the actual read
    return bme68x_read_u8(hi2c, address, reg);
}

void BME68x_Init(I2C_HandleType *hi2c)
{
    // Don't reinitialize I2C here - it's already done in main
    
    // Check if device is present first
    I2C_Start(hi2c);
    int device_present = I2C_SendAddress(hi2c, BME68X_ADDR, 0);
    I2C_Stop(hi2c);
    
    if (!device_present) {
        return; // Device not found, skip initialization
    }

    // Soft reset (datasheet: write 0xB6 to 0xE0)
    bme68x_write_u8(hi2c, BME68X_ADDR, 0xE0, 0xB6);
    delay_cycles(100000); // ~6ms at 168MHz (coarse)

    // Read and verify chip ID (0xD0 should be 0x61 for both BME680 and BME688)
    uint8_t chip_id = bme68x_read_u8(hi2c, BME68X_ADDR, 0xD0);
    (void)chip_id; // We'll check this in main function debug output
}

// Trigger a simple forced measurement with 1x temp/hum oversampling
static void bme68x_trigger_forced(I2C_HandleType *hi2c, uint8_t address)
{
    // Humidity oversampling x1
    bme68x_write_u8(hi2c, address, 0x72, 0x01);
    // Temperature oversampling x1, pressure skipped, mode = forced (01)
    bme68x_write_u8(hi2c, address, 0x74, 0x21);
...
static void bme68x_load_temp_calib(I2C_HandleType *hi2c, uint8_t address)
{
    if (s_cal_loaded) return;
    uint8_t t1_l = bme68x_read_u8(hi2c, address, 0xE9);
    uint8_t t1_h = bme68x_read_u8(hi2c, address, 0xEA);
    par_t1 = (uint16_t)((t1_h << 8) | t1_l);

    uint8_t t2_l = bme68x_read_u8(hi2c, address, 0x8A);
    uint8_t t2_h = bme68x_read_u8(hi2c, address, 0x8B);
    par_t2 = (int16_t)((t2_h << 8) | t2_l);

    par_t3 = (int8_t)bme68x_read_u8(hi2c, address, 0x8C);
    s_cal_loaded = 1;
}
...
int read_temp(I2C_HandleType *hi2c, uint8_t address)
{
    ...
    bme68x_trigger_forced(hi2c, address);
    bme68x_load_temp_calib(hi2c, address);
    // Optional status read (clears previous NEW_DATA bit); ignore value.
    (void)bme68x_read_u8(hi2c, address, 0x1D);

    // Read raw temperature ADC (msb@0x22, lsb@0x23, xlsb@0x24 upper nibble)
    uint32_t msb  = bme68x_read_u8(hi2c, address, 0x22);
    uint32_t lsb  = bme68x_read_u8(hi2c, address, 0x23);
    uint32_t xlsb = bme68x_read_u8(hi2c, address, 0x24);
...
int bme68x_get_cal_and_raw(I2C_HandleType *hi2c, uint8_t address, uint16_t *out_t1, int16_t *out_t2, int8_t *out_t3, uint32_t *out_adc_temp)
{
    ...
    *out_t3 = (int8_t)bme68x_read_u8(hi2c, address, 0x8C);

    // Trigger measurement and rely on fixed delay inside trigger
    bme68x_trigger_forced(hi2c, address);
    (void)bme68x_read_u8(hi2c, address, 0x1D); // clear status
    uint32_t msb  = bme68x_read_u8(hi2c, address, 0x22);
    uint32_t lsb  = bme68x_read_u8(hi2c, address, 0x23);
    uint32_t xlsb = bme68x_read_u8(hi2c, address, 0x24);
