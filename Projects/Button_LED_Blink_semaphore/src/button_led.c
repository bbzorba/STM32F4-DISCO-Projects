...
void EXTI0_IRQHandler(void) {
    ...
    EXTI->PR = (1U << 0);
    semaphore_give();  /* binary semaphore: idempotent -- bounce edges are harmless */
}
...
void blink_LEDS_sync(USART_HandleType *usart, LED_Type * const leds[]) {
    ...
    for (int cycle = 0; cycle < 5; ++cycle) {
        ...
        /* Mid-cycle probe: peek without consuming so async can take it normally. */
        if (g_semaphore > 0)
...
void blink_LEDS_async(USART_HandleType *usart, LED_Type * const leds[]) {
    ...
    if (!mutex_try_acquire()) {
        semaphore_give();  /* restore the count — press is not lost, will retry next call */
    ...
    /* semaphore_take() already decremented the count -- no manual clear needed. */
