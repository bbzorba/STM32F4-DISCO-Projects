...
void I2C_2_Init(I2C_SpeedType speed)
{
    ...
    I2C_2->CR1 |= I2C_CR1_PE;                                        // Disable I2C2
    ...
    I2C_2->CR2 = I2C_CR2_FREQ;                                       // Peripheral clock frequency
    if (speed == I2C_STANDARD_MODE)
    {
        I2C_2->CCR = 0x28;                                           // Standard mode, 100kHz
        I2C_2->TRISE = 0x09;                                         // Maximum rise time
    }
    else if (speed == I2C_FAST_MODE)
    {
        I2C_2->CCR = 0x0D;                                           // Fast mode, 400kHz
        I2C_2->TRISE = 0x03;                                         // Maximum rise time
    }

    I2C_2->CR1 |= I2C_CR1_PE;                                        // Enable I2C2
}

int read_I2C_address(int address)
{
    //1. wait until BUSY is reset in the SR2 register
    while(I2C_2->SR2 & I2C_SR2_BUSY);

    //2. generate START condition
    I2C_2->CR1 |= I2C_CR1_START;

    //3. wait until SB is set in the SR1 register
    while(!(I2C_2->SR1 & I2C_SR1_SB));

    //4. send address (left aligned) + R/W bit (1 for read)
    I2C_2->DR = (address << 1) | 1;

    //5. wait until ADDR is set in the SR1 register
    while(!(I2C_2->SR1 & I2C_SR1_ADDR));

    //6. clear ADDR by reading SR1 and SR2 registers
    volatile uint32_t temp = I2C_2->SR1;
    temp = I2C_2->SR2;
