...
{   
    ...
    GPIO_constructor(&CS_Handle, GPIO_A, &GPIO_InitStruct);
    ...
    SPI_Init(&spi1, SPI_1, SPI1_PORTA, SPI_MODE_MASTER, SPI_BAUDRATE_DIV16, SPI_DIRECTION_2LINES, SPI_CLOCK_POL_LOW_PHASE_1EDGE);
    ...
    while(1) {
        ...
        SPI_WriteReg(&spi1, &CS_Handle, GPIO_InitStruct.Pin, 0x0C, 0x5C); // Example write
        short_delay(1000);
        uint8_t reg_val = SPI_ReadReg(&spi1, &CS_Handle, GPIO_InitStruct.Pin, 0x0F, 0x80); // Example read from WHO_AM_I
